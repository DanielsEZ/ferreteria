...
<body>
  ...
  <form method="get" action="${pageContext.request.contextPath}/reportes/compras">
    <div class="row">
      <div>
        ...
        <select name="proveedorId">
          ...
          <c:forEach items="${proveedores}" var="p">
            <option value="${p.id}" ${param.proveedorId == p.id ? 'selected' : ''}>${p.nombre}</option>
          </c:forEach>
        </select>
      </div>
      <div>
    ...
    <div class="actions">
      ...
      <a class="button" href="${pageContext.request.contextPath}/reportes/compras/csv?${pageContext.request.queryString}">CSV</a>
